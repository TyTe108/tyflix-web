...
describe("plexServer.stopTranscodeSession", () => {
  it("GETs /video/:/transcode/universal/stop with session + user token", async () => {
    let requestedUrl: string | null = null;
    let requestedMethod: string | null = null;
    ...
    await client().stopTranscodeSession("sess-abc-123", "user-token-abc");
    ...
    assert.equal(
      ...
      `${BASE_URL}/video/:/transcode/universal/stop`,
    );
    assert.equal(url.searchParams.get("session"), "sess-abc-123");
    const headers = new Headers(requestedHeaders);
    assert.equal(headers.get("X-Plex-Token"), "user-token-abc");
  });
  ...
  it("throws PlexServerUpstreamError on a non-OK response", async () => {
    ...
    await assert.rejects(
      client().stopTranscodeSession("sess-abc-123", "user-token-abc"),
      (err: unknown) => {
        assert.ok(err instanceof PlexServerUpstreamError);
        assert.equal(err.status, 500);
        assert.match(err.message, /\/video\/:\/transcode\/universal\/stop/);
        return true;
      },
    );
  });
